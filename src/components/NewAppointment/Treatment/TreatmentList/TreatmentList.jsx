...
const TreatmentList = ({appointments, isLoading}) => {
    ...
    const data = appointments?.appointments
      .map(item => ({
        time: item.time,
        numberPatients: item.maxNumberPatients - (item.patients ? item.patients.length : 0)
      }));
              ...
              renderItem={item => (
                <List.Item className={TreatmentStyles.timesContainer}>
                  <Popover content={`Свободных мест: ${item.numberPatients}`}>
                    <Button
                      disabled={item.numberPatients < 1 || isToday && minutesOfDay(new Date()) > minutesOfDay(new Date(item.time))}
                    ...
                    >
                      ...
                      <Link to="/new-appointment/finish">{moment(item.time).format('H:mm')}</Link>
                    </Button>
                  </Popover>
                </List.Item>
              )}>
