...
api.interceptors.response.use(config => {
  ...
  if(error.response.status === 401 && error.config && !error.config._isRetry) {
    ...
    try {
      const data = await axios.get(process.env.REACT_APP_API_URL + '/auth/refresh', {withCredentials: true}).then(response => response.data);
...
export const authAPI = {
  isAuth() {
    return axios.get(process.env.REACT_APP_API_URL + '/auth/refresh', {withCredentials: true}).then(response => response.data);
  },
...
export const newAppointmentAPI = {
  getAppointments(date) {
    return api.get(`/appointments/by-date?date=${date}`).then(response => response.data);
  },
  addPatient(date, time, appointmentType, userId) {
    return api.put('/appointments/patients', {date, time, appointmentType, userId})
      .then(response => response.data);
  },
}
